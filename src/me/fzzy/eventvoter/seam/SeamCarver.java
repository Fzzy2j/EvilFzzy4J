package me.fzzy.eventvoter.seam;
...
public class SeamCarver {
	...
	/**
	 * Add a seam back to the picture and display it a specific color This method is
	 * used to display the seams that were removed
	 * 
	 * @param picture
	 * @param seam
	 * @param seamColor
	 *            the color that the seam will appear in
	 * @return
	 */
	public Picture addVerticalSeam(Picture picture, int[] seam, Color seamColor) {
		Picture newPicture = new ArrayPicture(picture.getWidth() + 1, picture.getHeight());
	 ...
	 * @return
	 */
	public Picture addHorizontalSeam(Picture picture, int[] seam, Color seamColor) {
		Picture newPicture = new ArrayPicture(picture.getWidth(), picture.getHeight() + 1);
