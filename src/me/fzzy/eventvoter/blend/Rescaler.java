package me.fzzy.eventvoter.blend;
...
public class Rescaler {
    ...
    public static BufferedImage rescaleImageWidth(BufferedImage imgIn, int numPixelsToRemove) {
    ...
    public static BufferedImage rescaleImageHeight(BufferedImage imgIn, int numPixelsToRemove) {
        imgIn = rotateImage(90, imgIn);
        int[][] minVarianceValue = getMinVarianceMatrix(imgIn);
        markBestPaths(minVarianceValue, numPixelsToRemove);
        return rotateImage(-90, rescaleImage(imgIn, minVarianceValue, numPixelsToRemove));
    }

    public static BufferedImage rotateImage(int rotate, BufferedImage bufferedImage) {
        AffineTransform affineTransform = new AffineTransform();

        affineTransform.translate(bufferedImage.getHeight() / 2, bufferedImage.getWidth() / 2);
        affineTransform.rotate(Math.toRadians(rotate));
    ...
    public static BufferedImage rescaleImageInSteps(BufferedImage imgIn, int numPixelsToRemove, int stepSize) {
        for (int a = 1; a < numPixelsToRemove; a += stepSize) {
            imgIn = rescaleImageWidth(imgIn, stepSize);
        }
        imgIn = rescaleImageWidth(imgIn, numPixelsToRemove % stepSize);
        return imgIn;
    }
}