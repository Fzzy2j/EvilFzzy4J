...
class MinesweeperChannel constructor(private val channelId: Long, reward: Int = 5) : ListenerAdapter() {
    ...
    init {
        ...
        val gameFile = File("games${File.separator}minesweeper${channelId}.json")
    ...
    override fun onMessageReceived(event: MessageReceivedEvent) {
        ...
        if (event.message.contentRaw.length >= 2 && !isOnCooldown(event.author.idLong)) {
            try {
                ...
                Bot.logger.info("[Minesweeper] ${event.member?.effectiveName} attacking ${event.message.contentRaw[0].toLowerCase()}${event.message.contentRaw.substring(1)}")
