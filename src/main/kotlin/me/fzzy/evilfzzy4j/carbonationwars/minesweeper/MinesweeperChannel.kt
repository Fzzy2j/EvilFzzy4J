...
class MinesweeperChannel constructor(private val channelId: Snowflake, cooldown: Int = 1000 * 60 * 60, reward: Int = 1) {
    ...
    private class Data constructor(
            ...
            @Expose var board: MinesweeperBoard = MinesweeperBoard(),
    ...
    init {
        val gameFile = File("games${File.separator}minesweeper${channelId.asLong()}.json")
        ...
        Bot.client.eventDispatcher.on(MessageCreateEvent::class.java).subscribe {
            if (it.message.channelId == channelId && !it.message.author.get().isBot) {
                if (it.message.content.get().length >= 2 && !isOnCooldown(it.member.get().id)) {
                    try {
                        if (data.board.attack(it.member.get().id, it.message.content.get()[0].toLowerCase(), it.message.content.get().substring(1).toInt())) {
                            val guild = FzzyGuild.getGuild(it.member.get().guildId)
                            guild.addCurrency(it.member.get(), reward)
                        }
                        Bot.logger.info("[Minesweeper] ${it.member.get().displayName} attacking ${it.message.content.get()[0].toLowerCase()}${it.message.content.get().substring(1)}")
        ...
        Bot.scheduler.schedulePeriodically({
            refreshCooldowns()
            if (data.board.isFinished()) {
                if (data.board.isWin()) {
                    val guild = FzzyGuild.getGuild(channel.guildId)
                    for (x in 0 until data.board.sizeX) {
                        for (y in 0 until data.board.sizeY) {
                            val who = data.board.whoAttacked('a' + x, y)
                            if (who != null) guild.addCurrency(who, 1)
                        }
                    }
                }
                data.board = MinesweeperBoard()
                refreshGame()
            }
